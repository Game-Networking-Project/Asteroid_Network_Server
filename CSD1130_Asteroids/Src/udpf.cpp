...
void udpf_impl::stream::process_incoming() {
	...
	const i64 max_dgram_payload_size{ static_cast<i64>(
			conn->config.max_datagram_size - stream_frame::transport_frame_header_size)
	};
...
void udpf_impl::stream::move_sliding_window_to_last_unack() {
	const i64 max_dgram_payload_size{ static_cast<i64>(conn->config.max_datagram_size) - static_cast<i64>(
			stream_frame::transport_frame_header_size)
	};
	const i64 wind_mod{ conn->config.max_window_size };
...
void udpf_impl::connection::send(char* data, u64 size) {
	...
	stream_frame osf{};
	const i64 max_dgram_payload_size{ static_cast<i64>(
			config.max_datagram_size - stream_frame::transport_frame_header_size)
	};
